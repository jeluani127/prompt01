...
║     by Pharmane | pharmane.ws/prompt01                       ║
...
def exportar_excel(dfs, stats_sensores, stats_batch, auditoria, riesgo, output_path):
    ...
    ws['A2'] = "Generado con Prompt Maestro 01 | pharmane.ws/prompt01"
...
def exportar_word(stats_sensores, stats_batch, auditoria, riesgo, imagenes, output_path):
    ...
    doc.add_paragraph()
    doc.add_paragraph("Generado con Prompt Maestro 01 | pharmane.ws/prompt01").italic = True
    ...
    run = footer_p.add_run("Prompt Maestro 01 | Pharmane | pharmane.ws/prompt01")
    run.font.size = Pt(8)
    run.italic = True
...
def exportar_pptx(stats_sensores, stats_batch, riesgo, imagenes, output_path):
    ...
    def add_slide_dark(title_text, subtitle_text="", content_lines=None, img_path=None):
        ...
        run_f = p_f.add_run()
        run_f.text = f"Prompt Maestro 01 | Pharmane | pharmane.ws/prompt01  |  {datetime.now().strftime('%d/%m/%Y')}  |  CONFIDENCIAL"
...
def main():
    ...
    print("\n  🧬 Prompt Maestro 01 | pharmane.ws/prompt01")
    print("═"*60 + "\n")
